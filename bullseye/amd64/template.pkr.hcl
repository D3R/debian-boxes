...
source "parallels-iso" "base-parallels" {
  boot_command = [
    "<esc><wait>",
    "auto url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/preseed.cfg hostname=${local.vm_name}",
    "<enter><wait>"
  ...
  iso_url                    = "${var.iso_url}"
  memory                     = 4096
  output_directory           = "output/base-parallels"
...
build {
  ...
  post-processors {
    post-processor "vagrant" {
      compression_level = 9
      output            = "output/base-{{ .Provider }}-amd64.box"
    }
    post-processor "checksum" {
      checksum_types = ["sha256"]
      output         = "output/base-parallels-amd64.box.checksum"
      only = [
        "parallels-iso.base-parallels"
      ]
    }
    post-processor "checksum" {
      checksum_types = ["sha256"]
      output         = "output/base-virtualbox-amd64.box.checksum"
      only = [
        "virtualbox-iso.base-virtualbox"
      ]
    }
    # post-processor "vagrant-cloud" {
    #   access_token        = "${var.cloud_token}"
    #   box_download_url    = "https://d3r-vagrant-stack.s3.eu-west-2.amazonaws.com/stable/base-{{ .Provider }}-amd64-${var.box_version}.box"
